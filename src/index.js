...
math.import({
  logicTable(...statements) {
    const O = [true, false];

    statements.forEach((s, k) => {
      rtv.ctx.save();
  ...
  scatter(points, psize = 8, colorFn) { // points [[x1, y1, z1], ...], psize, color([x,y,z])=[r,g,b] 0 <= r <= 1
    ...
    if (arguments.length === 3) {
      ...
      for (let j = 0; j < n; j++) {
        ...
        rtv.ctx.fillStyle = formatRgb(math.multiply(col, 255));
  ...
  point(a, size, color) { // point [x,y,z] size color[r,g,b]
    ...
    if (color) {
      ...
      rtv.ctx.fillStyle = formatRgb(math.multiply(constrained, 255).toArray());
    }
